...
function orchestrate(session, challengeDecision = {}, contradictionResult = {}, nextTopic = null) {
  ...
  logInfo('orchestrator', 'Orchestrating next action', {
    questionCount,
    currentStage,
    currentDifficulty,
    currentTopic,
  ...
  if (shouldEnd) {
    logInfo('orchestrator', 'Interview ending condition met', {
      questionCount,
      reason: questionCount >= MAX_QUESTIONS ? 'max_questions' : 'sufficient_signal'
    });
    return {
      shouldEnd: true,
      actionType: 'feedback',
  ...
  if (nextDifficulty !== currentDifficulty) {
    logInfo('orchestrator', 'Difficulty adapted', {
      from: currentDifficulty,
      to: nextDifficulty,
      recentScores: scores.slice(-2).map(s => s.score)
    });
  }

  // ── 3. Switch interviewer mode based on streaks ───────────────────────────
  const nextMode = resolveMode(consecutiveStrong, consecutiveWeak, questionCount);
  if (nextMode !== interviewMode) {
    logInfo('orchestrator', 'Interview mode changed', {
      from: interviewMode,
      to: nextMode,
      consecutiveStrong,
      consecutiveWeak
  ...
  if (unsurfacedContradiction) {
    logInfo('orchestrator', 'Action decided: surface contradiction', {
      contradictionId: unsurfacedContradiction.id,
      topic: currentTopic
    });
    return {
      shouldEnd: false,
  ...
  if (challengeDecision.shouldChallenge && questionsSinceChallenge >= 2) {
    logInfo('orchestrator', 'Action decided: challenge candidate', {
      challengeType: challengeDecision.type,
      challengeAngle: challengeDecision.angle,
      questionsSinceLastChallenge: questionsSinceChallenge
    });
    return {
      shouldEnd: false,
  ...
  logInfo('orchestrator', `Action decided: ${decision.actionType}`, {
    stage: nextStage,
    topic: resolvedNextTopic,
    difficulty: nextDifficulty,
    mode: nextMode,
    stageChanged,
    topicChanged
...
function checkShouldEnd(questionCount, scores, consecutiveStrong) {
  if (questionCount >= MAX_QUESTIONS) {
    logInfo('orchestrator', 'Max questions reached', { questionCount: MAX_QUESTIONS });
    return true;
  }
  if (questionCount < MIN_QUESTIONS) return false;

  // Enough signal: 6+ questions and last 3 were all strong (candidate proven)
  if (questionCount >= 6 && consecutiveStrong >= 3) {
    logInfo('orchestrator', 'Sufficient signal: strong performance streak', {
      questionCount,
      consecutiveStrong
    });
    return true;
  ...
  if (questionCount >= 7 && coveredAllStages) {
    logInfo('orchestrator', 'Sufficient signal: all stages covered', {
      questionCount,
