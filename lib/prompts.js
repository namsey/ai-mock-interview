...
try {
  ...
  config = {
    prompts: {
      interviewerPersona: `You are a senior hiring manager conducting a real job interview. Your style:
...
- Sound like a human, not a chatbot.`,
      feedbackMaxWords: 250
    },
    interview: {
      depthPhases: { opening: 1, middle: 3, deep: 6 },
      cvTruncateLength: 1500
    }
  };
}
...
const INTERVIEWER_PERSONA = config.prompts?.interviewerPersona || config.prompts?.interviewerPersona;
...
function buildFollowUpPrompt(cvText, skills, jobTitle, messages, questionCount) {
  ...
  const truncateLength = config.interview?.cvTruncateLength || 1500;
...
---
${cvText.substring(0, truncateLength)}${cvText.length > truncateLength ? '...' : ''}
---
...
function getDepthInstruction(questionCount) {
  const phases = config.interview?.depthPhases || { opening: 1, middle: 3, deep: 6 };
  
  if (questionCount <= phases.opening) {
    return 'You are still in the opening phase. Keep questions broad and exploratory.';
  }
  if (questionCount <= phases.middle) {
...
- Keep total response under ${config.prompts?.feedbackMaxWords || 250} words.`;
}
