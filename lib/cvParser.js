...
function extractCVInsights(cvText) {
  ...
  logInfo('cvParser', 'Starting CV parsing', { cvLength: cvText.length });
  ...
  if (allSkills.length === 0) {
    logWarn('cvParser', 'No skills detected in CV, using default', { cvLength: cvText.length });
    allSkills = ['Software Development'];
    skills.other = ['Software Development'];
  } else {
    logInfo('cvParser', 'Skills extracted', { 
  ...
  logInfo('cvParser', 'Experience signals detected', { 
    count: experienceSignals.length,
    signals: experienceSignals.slice(0, 5)
  });

  // 3. Estimate seniority
  const seniorityLevel = inferSeniority(cvText, lower);
  logInfo('cvParser', 'Seniority inferred', { seniorityLevel });

  // 4. Infer job title
  const jobTitle = inferJobTitle(lower, skills);
  logInfo('cvParser', 'Job title inferred', { jobTitle });

  // 5. Estimate years of experience
  const estimatedYears = estimateYearsOfExperience(cvText);
  logInfo('cvParser', 'Years of experience estimated', { estimatedYears });
  ...
  logInfo('cvParser', 'CV parsing completed', {
