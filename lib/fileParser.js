...
async function parsePdf(buffer) {
  ...
  let InvalidPDFException;

  try {
    const pdfParse = await import('pdf-parse');
    const { PDFParse } = pdfParse;
    PasswordException = pdfParse.PasswordException;
    InvalidPDFException = pdfParse.InvalidPDFException;
    ...
    if (message.includes('No text found')) throw err;
    if (PasswordException && err instanceof PasswordException) {
      throw new Error('PDF is password protected. Please remove the password or paste your CV text directly.');
    }
    if (InvalidPDFException && err instanceof InvalidPDFException) {
      throw new Error('Invalid PDF file. Please try exporting the PDF again or paste your CV text directly.');
    }
